...
func NewCatalog() (*Catalog, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("read embedded email locales: %w", err)
	}
	...
	for _, entry := range entries {
		...
		translation, err := readTranslation("locales/" + entry.Name())
		if err != nil {
			return nil, err
		}
...
func readTranslation(path string) (VerifyEmailI18n, error) {
	rawTranslation, err := embeddedLocales.ReadFile(path)
	if err != nil {
