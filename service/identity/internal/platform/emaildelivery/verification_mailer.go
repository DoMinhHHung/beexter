...
func NewVerificationMailer(
	...
	verificationURL, err := parseAbsoluteHTTPURL(
		rawVerificationURL,
	)
	if err != nil {
		return nil, err
	...
	messageIDDomain := strings.TrimSpace(
		sender.MessageIDDomain(),
	)
	if messageIDDomain == "" ||
		strings.ContainsAny(messageIDDomain, "\r\n@") {
		return nil, errors.New(
			"SMTP message ID domain is invalid",
		)
	}
...
func (m *VerificationMailer) SendVerification(
	...
	if ctx == nil ||
		...
		return errors.New(
			"verification email message is invalid",
		)
	}

	if err := validateVersion7ID(message.EventID); err != nil {
		return fmt.Errorf(
			"validate email event ID: %w",
			err,
		)
	}
	...
	if err := validateVersion7ID(message.TokenID); err != nil {
		return fmt.Errorf(
			"validate verification token ID: %w",
			err,
		)
	}
...
func validateVersion7ID(rawID string) error {
	...
	if parsedID.Version() != 7 ||
		...
		return errors.New(
			"ID must be a canonical UUID v7",
		)
	}
